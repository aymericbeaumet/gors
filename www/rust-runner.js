...
export class RustRunner {
	...
	_waitForBoot() {
		return this._waitForMarker(BOOT_READY_MARKER)
			.then(() => {
				this._serialBuffer = "";
				this._sendCommand(
	...
	_checkMarker() {
		...
		if (idx === -1) return;
		this._serialBuffer = this._serialBuffer.substring(
			idx + this._markerTarget.length,
		);
	...
	async compile(rustSource) {
		...
		if (
			...
			this._state !== State.COMPILING &&
			this._state !== State.RUNNING
		) {
			return {
				cancelled: false,
				compile: { success: false, stderr: `VM not ready (${this._state})` },
			};
		}

		this._setState(State.COMPILING);

		await this._emulator.create_file(
			`tmp/${jobId}.rs`,
		...
		const compileStatus = (
			await this._readFile(`tmp/${jobId}.compile.status`)
		).trim();
		const compileStderr = (
			await this._readFile(`tmp/${jobId}.compile.err`)
		).replace(ANSI_RE, "");
	...
	async runJob(jobId) {
		...
		const exitCode =
			parseInt((await this._readFile(`tmp/${jobId}.run.status`)).trim(), 10) ||
			0;
