...
const fixtureRoot = path.join(repoRoot, "tests", "fixtures", "go_programs");
...
async function addFixtureUsage(symbolsByPackage) {
	...
	const fixtureNames = entries
		.filter(
			(entry) => entry.isDirectory() && entry.name.startsWith("gostdlib_"),
		)
	...
	for (const fixtureName of fixtureNames) {
		...
		if (fixtureName === "gostdlib_builtin") {
...
// Run \`npm run generate:gostdlib-report\` from www/ after editing gostdlib fixtures.
