...
/// Print Go 1.26's shortest-round-trip `float32` representation.
///
/// The ABI carrier is `f64`; narrowing here ensures formatting observes the
/// exact source-language width even if a caller supplies a non-canonical
/// carrier.
pub fn print_f32(value: f64) {
    let rendered = format_f32_carrier(value);
    write_stderr_bytes(rendered.as_bytes());
}

fn format_f32_carrier(value: f64) -> String {
    format_f32(value as f32)
}
...
#[cfg(test)]
mod tests {
    use super::{format_f32, format_f32_carrier, format_f64};
    ...
    #[test]
    fn float32_format_matches_go_1_26_shortest_round_trip_output() {
        for (value, expected) in [
            (1.0_f32, "1"),
            (0.0, "0"),
            (-0.0, "-0"),
            ...
            assert_eq!(format_f32(value), expected, "{value:?}");
        }
        assert_eq!(format_f32(f32::NAN), "NaN");
        assert_eq!(
            format_f32_carrier(core::f64::consts::E),
            "2.7182817",
            "the physical f64 carrier must narrow before float32 formatting",
        );
    }
}
