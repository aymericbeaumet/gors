...
| display impl pruning mixed into `compiler/mod.rs` | Display impl retention now lives in `compiler/display_impls.rs`, where inherent `String` methods and `std::error::Error` impls explicitly define which generated Display impls remain reachable. |
