...
| noop interface item assembly mixed into `compiler/mod.rs` | Noop interface structs, noop trait impl bodies, and noop supertrait impl traversal now live in `compiler/noop_interfaces.rs`; `compiler/mod.rs` only supplies TypeEnv-derived external interface method signatures. |
