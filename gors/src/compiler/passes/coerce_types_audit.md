...
| `gors/src/compiler/mod.rs` | `strconv.AppendFloat` custom lowering to `builtin::append_float`, isolated behind `StdlibCallWorkaround` | stdlib workaround | Compile `strconv` through `internal/strconv` normally after fixing generated package-var roots, unsized slice field assignments, large integer constant lowering, and reflect fallback pruning. |
