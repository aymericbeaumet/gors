...
| `pp` receiver-name gate for `self.value` reflection fallback pruning | The fallback prune now uses the same generated receiver metadata as flush insertion, so receiver types are selected by `__gors_flush_fmt` plus flushable print methods instead of the literal type name `pp`. |
