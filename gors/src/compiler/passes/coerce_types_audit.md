...
| runtime interface hook policy mixed into `compiler/mod.rs` | Runtime hook names, clone-box impl generation, noop hook detection, and missing clone-hook patching now live in `compiler/interface_hooks.rs`; compiler lowering and reachability consume that generated-interface contract through one focused boundary. |
