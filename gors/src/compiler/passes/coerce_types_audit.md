...
| interface method-set discovery mixed into `compiler/mod.rs` | Direct/imported/embedded interface method expansion and value/pointer satisfaction predicates now live in `compiler/interface_method_sets.rs`; the compiler root consumes method-set facts when orchestrating impl emission. |
